...
public class HeapPQ<K,V> {
   ...
   // returns an entry with minimal key.
    ...
    public void merge(HeapPQ<K,V> other) {
    ...
    public void replace(int value1, int value2) {
        int index = -1;
        for (int i = 0; i < heap.size(); i++) {
            if (heap.get(i).getValue() == value1) {
                index = i;
                break;
            }
        }
        if (index != -1) {
            heap.get(index).setValue(value2);
            upheap(index);
            downheap(index);
        }
    }

    public boolean isConsecutive(PriorityQueue<Integer> queue) {
